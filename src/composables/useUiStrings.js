...
// from the data package (decision G3). Each file is the shared MWNF online
// exhibition layer with this exhibition's own keys overlaid: the curated
// introductions (`txtCollection`, `txtPartners`, `txtTimeline`,
// `txtStaticHomepage`), the credits, and the logo-section headings.
//
// These are legacy content, not website copy: they are generated in the
// inventory-app monorepo (`scripts/viewers/the-use-of-colours-in-art`,
// `npm run sync-i18n`) and vendored here. Do not hand-edit them — the
// website's own translatable texts live in `locales/`, which is what the
// translator flow edits.
//
...
// The chrome language. An exhibition is a per-language build, so this never
// moves: the site ships English and the header carries no switcher. It stays a
// ref because the item sheet and the partner profile still switch on the
// *record's* language, exactly as the legacy client did.
